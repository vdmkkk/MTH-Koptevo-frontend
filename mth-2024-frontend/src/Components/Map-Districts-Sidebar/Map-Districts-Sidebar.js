...
function MapDistrictsSidebar({district, open, setOpen, setPopupOpen}) {

    if (district !== undefined)
    return(
        <div className={open ? "main-sidebar sidebar-open" : "main-sidebar sidebar-closed"}>
            <h1>{district["name"]}</h1>
            <div className='tag-container'>{district["properties"]["tags"].map(tag => {
                return <div className='tag'><p>{tag}</p></div>
            })}</div>
            <h2>{district["properties"]["description"]}</h2>
            ...
            <div className='localComment'>
                <div className='top'>
                    ...
                    <div>
                        <h1>{district["properties"]["localComment"]["name"]}</h1>
                        <h2>Гид</h2>
                    </div>
                </div>
                <h2>{district["properties"]["localComment"]["text"]}</h2>
                <div className='line'/>
            </div>
            <div onClick={() => {setOpen(district["id"]); setPopupOpen(false);}} className='button'>
                <p>Выбрать район</p>
            </div>
        </div>
    ) 
